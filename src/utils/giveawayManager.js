...
async function endGiveaway(client, giveaway) {
    ...
    try {
        ...
        try {
            ...
            let reaction = null;
            if (emojiId) reaction = message.reactions.cache.get(emojiId);
            if (!reaction) reaction = message.reactions.cache.find(r => r.emoji.name === 'giveaway');

            if (reaction) {
                const users = await reaction.users.fetch();
                const reactionParticipants = users.filter(u => !u.bot).map(u => u.id);
                const merged = new Set([...(giveaway.participants || []), ...reactionParticipants]);
                giveaway.participants = [...merged];
                client.db.giveaways.set(giveaway.messageId, giveaway);
            }
        } catch (e) {
            console.error('[Giveaway] Failed to refresh reactions before end:', e);
        }
