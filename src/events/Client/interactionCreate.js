...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    if (interaction.isModalSubmit()) {
      ...
      if (interaction.customId.startsWith('tsm_')) {
        try { const cmd = client.commands.get('ticket'); if (cmd) await cmd.modalHandler(interaction, client); } catch (e) { console.error('Ticket settings modal error:', e); }
      }
      if (interaction.customId.startsWith('ticket_modal_')) {
        try { const cmd = client.commands.get('ticket'); if (cmd) await cmd.ticketModalHandler(interaction, client); } catch (e) { console.error('Ticket open modal error:', e); }
      }
    }
