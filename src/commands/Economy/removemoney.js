...
module.exports = {
  ...
  async _run(ctx, target, amount, client) {
    ...
    return ctx.reply({ components: [new ContainerBuilder().addTextDisplayComponents(new TextDisplayBuilder().setContent(`### ✅ Coins Removed\n-\`${formatNumber(amount)}\` from **${target.username}**\n> 👛 New Wallet: \`${formatNumber(eco.wallet)}\`\n-# Chill Economy`))], flags: MessageFlags.IsComponentsV2 });
  }
};
