...
module.exports = {
  ...
  aliases: ['lol'],
  category: 'Reaction',
  description: 'Laugh with a funny anime GIF',
  cooldown: 3,
  slashOptions: [],
  ...
  async _run(ctx, client) {
    ...
    const { url } = await fetchGif('laugh');
    const media = new MediaGalleryBuilder().addItems(new MediaGalleryItemBuilder().setURL(url));
    const container = new ContainerBuilder()
      .addTextDisplayComponents(new TextDisplayBuilder().setContent(`### ${emoji.laugh} LMAO!\n**${author.username}** is dying of laughter! 😂\n-# Powered by Chill • Developed by AkiForver`))
