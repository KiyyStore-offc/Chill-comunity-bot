...
module.exports = {
  ...
  cooldown: 3,
  slashOptions: [{ name: 'user', description: 'User to pat', type: 6, required: false }],
  ...
  async _run(ctx, target, client) {
    ...
    const text = isSelf ? `**${author.username}** pats their own head! 🫶` : `**${author.username}** gives **${target.username}** a head pat! 🫶`;
    const media = new MediaGalleryBuilder().addItems(new MediaGalleryItemBuilder().setURL(url));
    const container = new ContainerBuilder()
      .addTextDisplayComponents(new TextDisplayBuilder().setContent(`### ${emoji.pat} Head Pat!\n${text}\n-# Powered by Chill • Developed by AkiForver`))
