...
module.exports = {
  ...
  cooldown: 3,
  slashOptions: [{ name: 'user', description: 'User to yeet', type: 6, required: false }],
  ...
  async _run(ctx, target, client) {
    ...
    const { url } = await fetchGif('yeet');
    const text = target && target.id !== author.id ? `**${author.username}** YEETED **${target.username}** into orbit! 🚀` : `**${author.username}** yeeted themselves! 🚀`;
    const media = new MediaGalleryBuilder().addItems(new MediaGalleryItemBuilder().setURL(url));
    const container = new ContainerBuilder()
      .addTextDisplayComponents(new TextDisplayBuilder().setContent(`### 🚀 YEET!\n${text}\n-# Powered by Chill • Developed by AkiForver`))
