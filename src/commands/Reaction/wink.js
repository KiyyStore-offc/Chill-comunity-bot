...
module.exports = {
  ...
  cooldown: 3,
  slashOptions: [{ name: 'user', description: 'User to wink at', type: 6, required: false }],
  ...
  async _run(ctx, target, client) {
    ...
    const { url } = await fetchGif('wink');
    const text = target && target.id !== author.id ? `**${author.username}** winks at **${target.username}**! 😉` : `**${author.username}** winks! 😉`;
    const media = new MediaGalleryBuilder().addItems(new MediaGalleryItemBuilder().setURL(url));
    const container = new ContainerBuilder()
      .addTextDisplayComponents(new TextDisplayBuilder().setContent(`### ${emoji.wink} Wink!\n${text}\n-# Powered by Chill • Developed by AkiForver`))
