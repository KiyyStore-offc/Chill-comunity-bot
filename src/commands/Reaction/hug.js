...
module.exports = {
  name: 'hug',
  category: 'Reaction',
  description: 'Hug someone with a cute anime GIF',
  cooldown: 3,
  slashOptions: [{ name: 'user', description: 'User to hug', type: 6, required: false }],

  async slashExecute(interaction, client) {
    const target = interaction.options.getUser('user');
    return this._run({ author: interaction.user, guild: interaction.guild, reply: async (o) => interaction.reply(o) }, target, client);
  },
  ...
  async _run(ctx, target, client) {
    ...
    const text = isSelf ? `**${author.username}** hugs themselves! 🤗` : `**${author.username}** hugs **${target.username}**! 🤗`;
    const media = new MediaGalleryBuilder().addItems(new MediaGalleryItemBuilder().setURL(url));
    const container = new ContainerBuilder()
      .addTextDisplayComponents(new TextDisplayBuilder().setContent(`### ${emoji.hug} Hug!\n${text}\n-# Powered by Chill • Developed by AkiForver`))
