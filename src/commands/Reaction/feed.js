...
module.exports = {
  name: 'feed',
  category: 'Reaction',
  description: 'Feed someone with a cute anime GIF',
  cooldown: 3,
  slashOptions: [{ name: 'user', description: 'User to feed', type: 6, required: false }],
  ...
  async _run(ctx, target, client) {
    ...
    const { url } = await fetchGif('feed');
    const text = target && target.id !== author.id ? `**${author.username}** feeds **${target.username}**! 🍖` : `**${author.username}** eats alone... 🍖`;
    const media = new MediaGalleryBuilder().addItems(new MediaGalleryItemBuilder().setURL(url));
    const container = new ContainerBuilder()
      .addTextDisplayComponents(new TextDisplayBuilder().setContent(`### ${emoji.feed} Feed!\n${text}\n-# Powered by Chill • Developed by AkiForver`))
