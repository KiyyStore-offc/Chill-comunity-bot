...
module.exports = {
  ...
  aliases: ['rage'],
  category: 'Reaction',
  description: 'Show your anger with an anime GIF',
  cooldown: 3,
  slashOptions: [{ name: 'user', description: 'User you are angry at', type: 6, required: false }],
  ...
  async _run(ctx, target, client) {
    ...
    const { url } = await fetchGif('angry');
    const text = target && target.id !== author.id ? `**${author.username}** is angry at **${target.username}**! 😠` : `**${author.username}** is FURIOUS! 😠`;
    const media = new MediaGalleryBuilder().addItems(new MediaGalleryItemBuilder().setURL(url));
    const container = new ContainerBuilder()
      .addTextDisplayComponents(new TextDisplayBuilder().setContent(`### ${emoji.angry} ANGRY!\n${text}\n-# Powered by Chill • Developed by AkiForver`))
