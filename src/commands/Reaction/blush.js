...
module.exports = {
  name: 'blush',
  category: 'Reaction',
  description: 'Show off your blush with an anime GIF',
  cooldown: 3,
  slashOptions: [],
  ...
  async _run(ctx, client) {
    ...
    const { url } = await fetchGif('blush');
    const media = new MediaGalleryBuilder().addItems(new MediaGalleryItemBuilder().setURL(url));
    const container = new ContainerBuilder()
      .addTextDisplayComponents(new TextDisplayBuilder().setContent(`### ${emoji.blush} Blush!\n**${author.username}** is blushing! 😊\n-# Powered by Chill • Developed by AkiForver`))
