...
module.exports = {
  name: 'lick',
  category: 'Reaction',
  description: 'Lick someone (you weirdo)',
  cooldown: 3,
  slashOptions: [{ name: 'user', description: 'User to lick', type: 6, required: false }],
  ...
  async _run(ctx, target, client) {
    ...
    const { url } = await fetchGif('lick');
    const isSelf = !target || target.id === author.id;
    const text = isSelf ? `**${author.username}** licks themselves... 👅` : `**${author.username}** licks **${target.username}**! 👅`;
    const media = new MediaGalleryBuilder().addItems(new MediaGalleryItemBuilder().setURL(url));
    const container = new ContainerBuilder()
      .addTextDisplayComponents(new TextDisplayBuilder().setContent(`### ${emoji.lick} Lick!\n${text}\n-# Powered by Chill • Developed by AkiForver`))
