...
module.exports = {
  ...
  cooldown: 3,
  slashOptions: [{ name: 'user', description: 'User to cuddle', type: 6, required: false }],
  ...
  async _run(ctx, target, client) {
    ...
    const text = isSelf ? `**${author.username}** cuddles a pillow 🥰` : `**${author.username}** cuddles with **${target.username}**! 🥰`;
    const media = new MediaGalleryBuilder().addItems(new MediaGalleryItemBuilder().setURL(url));
    const container = new ContainerBuilder()
      .addTextDisplayComponents(new TextDisplayBuilder().setContent(`### ${emoji.cuddle} Cuddle!\n${text}\n-# Powered by Chill • Developed by AkiForver`))
