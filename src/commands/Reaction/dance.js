...
module.exports = {
  ...
  cooldown: 3,
  slashOptions: [{ name: 'user', description: 'User to dance with', type: 6, required: false }],
  ...
  async _run(ctx, target, client) {
    ...
    const { url } = await fetchGif('dance');
    const text = target && target.id !== author.id ? `**${author.username}** dances with **${target.username}**! 💃` : `**${author.username}** is dancing! 💃`;
    const media = new MediaGalleryBuilder().addItems(new MediaGalleryItemBuilder().setURL(url));
    const container = new ContainerBuilder()
      .addTextDisplayComponents(new TextDisplayBuilder().setContent(`### 💃 Dance!\n${text}\n-# Powered by Chill • Developed by AkiForver`))
