...
module.exports = {
  ...
  cooldown: 3,
  slashOptions: [{ name: 'user', description: 'User to wave at', type: 6, required: false }],
  ...
  async _run(ctx, target, client) {
    ...
    const { url } = await fetchGif('wave');
    const text = target && target.id !== author.id ? `**${author.username}** waves at **${target.username}**! 👋` : `**${author.username}** waves! 👋`;
    const media = new MediaGalleryBuilder().addItems(new MediaGalleryItemBuilder().setURL(url));
    const container = new ContainerBuilder()
      .addTextDisplayComponents(new TextDisplayBuilder().setContent(`### ${emoji.wave} Wave!\n${text}\n-# Powered by Chill • Developed by AkiForver`))
