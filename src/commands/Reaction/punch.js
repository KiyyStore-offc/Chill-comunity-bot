...
module.exports = {
  name: 'punch',
  category: 'Reaction',
  description: 'Punch someone anime style',
  cooldown: 3,
  slashOptions: [{ name: 'user', description: 'User to punch', type: 6, required: false }],
  ...
  async _run(ctx, target, client) {
    ...
    const { url } = await fetchGif('punch');
    const isSelf = !target || target.id === author.id;
    const text = isSelf ? `**${author.username}** punches themselves?! 👊` : `**${author.username}** punches **${target.username}**! 👊`;
    const media = new MediaGalleryBuilder().addItems(new MediaGalleryItemBuilder().setURL(url));
    const container = new ContainerBuilder()
      .addTextDisplayComponents(new TextDisplayBuilder().setContent(`### ${emoji.punch} Punch!\n${text}\n-# Powered by Chill • Developed by AkiForver`))
