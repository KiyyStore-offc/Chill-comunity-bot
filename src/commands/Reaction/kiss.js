...
module.exports = {
  ...
  cooldown: 3,
  slashOptions: [{ name: 'user', description: 'User to kiss', type: 6, required: false }],
  ...
  async _run(ctx, target, client) {
    ...
    const text = isSelf ? `**${author.username}** kisses the air... 💋` : `**${author.username}** kisses **${target.username}**! 💋`;
    const media = new MediaGalleryBuilder().addItems(new MediaGalleryItemBuilder().setURL(url));
    const container = new ContainerBuilder()
      .addTextDisplayComponents(new TextDisplayBuilder().setContent(`### ${emoji.kiss} Kiss!\n${text}\n-# Powered by Chill • Developed by AkiForver`))
