...
module.exports = {
  ...
  cooldown: 3,
  slashOptions: [{ name: 'user', description: 'User to slap', type: 6, required: false }],
  ...
  async _run(ctx, target, client) {
    ...
    const text = isSelf ? `**${author.username}** slaps themselves!? 👋` : `**${author.username}** slaps **${target.username}**! 👋`;
    const media = new MediaGalleryBuilder().addItems(new MediaGalleryItemBuilder().setURL(url));
    const container = new ContainerBuilder()
      .addTextDisplayComponents(new TextDisplayBuilder().setContent(`### ${emoji.slap} Slap!\n${text}\n-# Powered by Chill • Developed by AkiForver`))
